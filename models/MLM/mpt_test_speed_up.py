...
class VisualBertPromptModel(nn.Module):
    def __init__(self, prefix_prompt_num, prompt_candidates, predicates_words, hidden_size=768, relation_type_count=31):
        ...
        #         time_0 = time.time()
        #         predicate_embedding = self.tokenizer.encode(word, return_tensors="pt", add_special_tokens = False)
        #         len_encode = predicate_embedding.shape[1]
        #         length_encode_list.append(len_encode)
        #         time_1 = time.time()
        #         predicate_embedding = self.embedding_input(predicate_embedding)
        #         time_2 = time.time()
        #         predicate_embedding = torch.mean(predicate_embedding,dim=1) # torch.mean还是有用的
        #         time_3 = time.time()
        #         cost_0 = time_1 - time_0
        #         cost_1 = time_2 - time_1
        #         cost_2 = time_3 - time_2
        #         total_cost_0 = total_cost_0 + cost_0
        #         total_cost_1 = total_cost_1 + cost_1
        #         total_cost_2 = total_cost_2 + cost_2
        ...
        #         self.predicate_embeddings.append(predicate_embedding)       #遍历word_table 中的每个单词，使用模型将每个单词编码成嵌入向量，并将这些嵌入向量存储在 self.predicate_embeddings 列表中，以供后续的处理使用
        # a = 0
        # for i in range(len(self.predicate_embeddings_fast)):
        #     if torch.sum(self.predicate_embeddings_fast[i]==self.predicate_embeddings[i])==768:
        #         a += 1
    ...
    def forward(self, batch_text, batch_img, weight=None, theta=1.0, device='cuda', is_label=True):
        ...
        contexts = []
        image_in = batch_img[0].to(device)
        #############################################
        ...
        image_in = {'pixel_values':image_in,'output_attentions':True}
        ...
        output = self.model(input_ids=text_input, labels=label, visual_prompts=visual_prompt, attention_mask=attention_mask)   ##Bertmodel    只使用hidden_states作后续处理          
        mask_pos = torch.Tensor(mask_pos).long()                          ##[64,1]
        
        weight = torch.Tensor(weight).to(device) if weight is not None else None
        if not is_label:
            label = None
        final = self.prompt4re(output[1], mask_pos=mask_pos, labels=label, weight=weight, theta=theta, device=device)      #####loss+logits
